...
1. `mkdir -p eval/private && cp eval/queries.example.ts eval/private/queries.ts`
   and fill in ~15 known-answer queries against YOUR indexed history (session
   IDs from `bun run src/cli.ts search` / `stats`), plus 3+ off-corpus negative
   controls. Everything under `eval/private/` is gitignored — it references
   your private sessions.
2. `bun run eval/build-corpus.ts` — snapshots your history into
   `eval/private/corpus.json` using the exact production chunking, so all
...
the test set, and writes `eval/private/results-<modelKey>.json`. Compare with
your favorite script: top-1/top-3 hit rate over positives, and worst-case
